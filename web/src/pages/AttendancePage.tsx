...
type AttendanceMetric = 'all' | 'present' | 'absent' | 'leave' | 'holiday' | 'wfh' | 'late';
...
export default function AttendancePage() {
  ...
  const todayDetailRows = useMemo(() => employees.map((employee) => {
    const sessions = records
      .filter((record) => record.employee_id === employee.id && record.date === selectedDate)
      .sort((a, b) => (a.check_in ?? '').localeCompare(b.check_in ?? ''));
    ...
    const approvedLeave = leaveRequests.some((request) => request.employee_id === employee.id && request.status === 'APPROVED' && selectedDate >= request.start_date && selectedDate <= request.end_date);
    const present = sessionsWithCheckIn.length > 0;
    const holiday = classifyDay(selectedDate, holidays) === 'HOLIDAY';
    ...
    return {
      ...
      workedMinutes,
      status: approvedLeave ? 'On leave' : present ? (late ? 'Late' : 'Present') : holiday ? 'Holiday' : 'Absent',
    };
  }), [employees, records, selectedDate, leaveRequests, holidays, departments, designations]);
  ...
  async function handleMetricSelect(metric: AttendanceMetric) {
    await Promise.all([refreshAttendance(), refreshLeaveRequests()]);
    setSelectedMetric(metric === 'all' ? null : metric);
  }

  const overviewRows = useMemo(() => todayDetailRows.filter((row) => {
    const search = searchTerm.trim().toLowerCase();
    const metricOk = selectedMetric === null || selectedMetric === 'all'
      || (selectedMetric === 'present' && row.present)
      || (selectedMetric === 'absent' && !row.present && !row.approvedLeave && !row.holiday)
      || (selectedMetric === 'leave' && row.approvedLeave)
      || (selectedMetric === 'holiday' && row.holiday && !row.present && !row.approvedLeave)
      || (selectedMetric === 'wfh' && row.wfh)
      || (selectedMetric === 'late' && row.late);
    return metricOk
      && (departmentFilter === 'ALL' || row.employee.department_id === departmentFilter)
      && (managerFilter === 'ALL' || row.employee.manager_id === managerFilter)
      && (organizationWorkMode === 'ALL' || row.employee.work_mode === organizationWorkMode)
      && (statusFilter === 'ALL' || row.status === statusFilter)
      && (!search || row.employee.name.toLowerCase().includes(search));
  }), [todayDetailRows, selectedMetric, departmentFilter, managerFilter, organizationWorkMode, statusFilter, searchTerm]);
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="inline-flex items-center rounded-md border p-1" style={{ borderColor: 'var(--line-soft)', background: 'var(--paper)' }}>
            {['Overview', 'Monthly Employees Attendance'].map((label) => {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border bg-white p-4" style={{ borderColor: 'var(--line-soft)', borderRadius: 'var(--radius-md)' }}>
          ...
          <div className="flex gap-2">
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="border px-2.5 py-2 text-sm" style={{ borderColor: 'var(--line)', borderRadius: 'var(--radius-sm)' }} aria-label="Attendance date" />
        ...
        {/* UPDATED: Organization Overview Table - TODAY'S ATTENDANCE DETAILS */}
        <div className="border bg-white" style={{ borderColor: 'var(--line-soft)', borderRadius: 'var(--radius-md)' }}>
          <div className="border-b p-4" style={{ borderColor: 'var(--line-soft)' }}>
            <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-5">
              ...
              <select
                value={organizationWorkMode}
                onChange={(e) => setOrganizationWorkMode(e.target.value as 'ALL' | WorkMode)}
                className="border px-3 py-2 text-sm"
                style={{ borderColor: 'var(--line)', borderRadius: 'var(--radius-sm)' }}
              >
                ...
                <tbody>
                  {overviewRows.map((row) => {
                    ...
                    return (
                      <tr
                        key={row.employee.id}
                        onClick={() => navigate(`/attendance/${row.employee.id}?month=${month}&year=${year}`)}
                        className="cursor-pointer border-t transition-colors hover:bg-[var(--paper)]"
                        style={{ borderColor: 'var(--line-soft)' }}
                      >
                        {/* Employee Name */}
                        <td className="px-5 py-3 text-sm font-medium" style={{ color: 'var(--ink)' }}>
                          {row.employee.name}
                        </td>
                        ...
                        <td
                          ...
                          style={{ color: row.present ? 'var(--ink)' : 'var(--text-muted)' }}
                        >
                          {row.firstSession?.check_in ?? '—'}
                        </td>

                        {/* Check-out Time */}
                        <td
                          className="px-5 py-3 font-mono text-xs"
                          style={{ color: row.lastSession?.check_out ? 'var(--ink)' : 'var(--text-muted)' }}
                        >
                          {row.lastSession?.check_out ?? '—'}
                        </td>

                        {/* Hours Worked */}
                        <td className="px-5 py-3 font-mono text-xs" style={{ color: 'var(--ink)' }}>
                        ...
                        {/* Manual Entry Reason */}
                        <td
                          className="px-5 py-3 text-xs truncate max-w-xs"
                          style={{ color: row.firstSession?.manual_entry_reason ? 'var(--ink)' : 'var(--text-muted)' }}
                          title={row.firstSession?.manual_entry_reason ?? undefined}
                        >
                          {row.firstSession?.manual_entry_reason ?? '—'}
                        </td>
                      </tr>
                    );
                  })}
          ...
          {/* Employee Attendance Section (Monthly View) */}
          {superAdminSection === 'employees' ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] text-left">
                <thead>
                  <tr style={{ background: 'var(--paper)' }}>
                    {['Employee', 'Department', 'Required', 'Worked', 'Overtime', 'Shortfall', 'Attendance', ''].map((heading) => (
                      <th key={heading} className="px-5 py-3 text-xs uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>
                        {heading}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {organizationRows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
                        {departments.find((department) => department.id === row.employee.department_id)?.name ?? 'Unknown'}
                      </td>
                      <td className="px-5 py-3 font-mono text-xs">{formatMinutes(row.requiredMinutes)}</td>
                      <td className="px-5 py-3 font-mono text-xs">{formatMinutes(row.workedMinutes)}</td>
                      <td className="px-5 py-3 font-mono text-xs" style={{ color: row.overtime > 0 ? 'var(--status-present)' : 'var(--text-muted)' }}>
                        {formatMinutes(row.overtime)}
                      </td>
                      <td className="px-5 py-3 font-mono text-xs" style={{ color: row.shortfall > 0 ? 'var(--status-absent)' : 'var(--text-muted)' }}>
                        {formatMinutes(row.shortfall)}
                      </td>
                      <td className="px-5 py-3">
                        <span className="font-mono text-xs font-semibold" style={{ color: row.attendance >= 90 ? 'var(--status-present)' : 'var(--status-absent)' }}>
                          {row.attendance.toFixed(1)}%
                        </span>
                        <span className="ml-2 text-[10px] uppercase" style={{ color: row.todayStatus === 'LEAVE' ? '#F59E0B' : row.todayStatus === 'UPCOMING' ? 'var(--text-muted)' : 'var(--text-secondary)' }}>
                          {row.todayStatus}
                        </span>
                      </td>
