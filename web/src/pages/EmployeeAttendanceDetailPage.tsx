...
export default function EmployeeAttendanceDetailPage() {
    ...
    function handleManualSave(event: React.FormEvent) {
        ...
        if (!employee) return;
        if (!manualForm.manual_entry_reason.trim() || !manualForm.work_summary.trim()) return;
    ...
    function saveEdit(event: React.FormEvent) {
        ...
        const workedMinutes = Math.round((hoursBetween(editForm.check_in || null, editForm.check_out || null) ?? 0) * 60);
        const overtimeMinutes = Math.max(0, workedMinutes - OVERTIME_THRESHOLD_MINUTES);
        const earlyCheckout = workedMinutes < EARLY_CHECKOUT_THRESHOLD_MINUTES;
    ...
    if (role === 'SUPER_ADMIN' || role === 'HR') {
        ...
        return (
            <div className="mx-auto max-w-6xl space-y-6">
                <ConfirmDialog open={approvalAction !== null} message={approvalAction?.action === 'approve' ? 'Are you sure you want to approve this record?' : 'Are you sure you want to reject this record?'} onCancel={() => setApprovalAction(null)} onConfirm={() => { if (approvalAction?.action === 'approve') approveRecord(approvalAction.id); if (approvalAction?.action === 'reject') rejectRecord(approvalAction.id); setApprovalAction(null); }} />
        ...
        <div className="mx-auto max-w-5xl space-y-6">
            ...
            {manualMode && (
                <form onSubmit={handleManualSave} className="grid gap-3 border bg-white p-4 md:grid-cols-5" style={{ borderColor: 'var(--line-soft)', borderRadius: 'var(--radius-md)' }}>
                    ...
                    <input required value={manualForm.manual_entry_reason} onChange={(e) => setManualForm((prev) => ({ ...prev, manual_entry_reason: e.target.value }))} placeholder="Reason for manual entry" className="border px-3 py-2 text-sm md:col-span-2" style={{ borderColor: 'var(--line)', borderRadius: 'var(--radius-sm)' }} />
                    <textarea required value={manualForm.work_summary} onChange={(e) => setManualForm((prev) => ({ ...prev, work_summary: e.target.value }))} placeholder="What did they work on this day?" rows={2} className="w-full border px-3 py-2 text-sm md:col-span-2" style={{ borderColor: 'var(--line)', borderRadius: 'var(--radius-sm)' }} />
                    {Math.round((hoursBetween(manualForm.check_in, manualForm.check_out) ?? 0) * 60) > OVERTIME_THRESHOLD_MINUTES && <input required value={manualForm.overtime_reason} onChange={(e) => setManualForm((prev) => ({ ...prev, overtime_reason: e.target.value }))} placeholder="Overtime reason" className="border px-3 py-2 text-sm md:col-span-2" style={{ borderColor: 'var(--line)', borderRadius: 'var(--radius-sm)' }} />}
